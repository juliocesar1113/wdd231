...
export function renderStars(rating) {
  ...
  let html = '<div class="ab-stars" aria-hidden="true">';
  for (let i = 1; i <= 5; i++) {
    if (i <= fullStars) {
      html += `<svg class="ab-star" viewBox="0 0 24 24" fill="currentColor" stroke="none"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/></svg>`;
    } else if (i === fullStars + 1 && hasHalf) {
      html += `<svg class="ab-star" viewBox="0 0 24 24" fill="currentColor" stroke="none"><defs><linearGradient id="h${i}"><stop offset="50%" stop-color="currentColor"/><stop offset="50%" stop-color="transparent" stop-opacity="1"/></linearGradient></defs><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" fill="url(#h${i})" stroke="currentColor" stroke-width="1"/></svg>`;
    } else {
      html += `<svg class="ab-star" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/></svg>`;
    }
  }
  html += '</div>';
  return html;
